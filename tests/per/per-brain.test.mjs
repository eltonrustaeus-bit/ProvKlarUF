...
/* api/_per-graph-data.js genereras av tools/build-per-graph.mjs och läses av
   ...
   arkitektur — precis den falska trygghet registret finns för att förhindra.

   Filen genereras i stället för att läsas vid körning eftersom
   dirname(fileURLToPath(import.meta.url)) i api/admin.js tog ned hela
   adminpanelen 2026-08-25: Vercel laddar filen som CJS, och import.meta är
   ett syntaxfel där. Se tests/api/cjs-esm-boundary.test.mjs. */
const { PER_GRAPH } = await import(join(apiDir, "_per-graph-data.js"));
const utanGenererad = { ...filer };
delete utanGenererad["_per-graph-data.js"];
const färsk = B.buildGraph(utanGenererad);

check("den genererade grafen har samma noder som källan",
  JSON.stringify(PER_GRAPH.noder) === JSON.stringify(färsk.noder),
  `genererad ${PER_GRAPH.noder.length}, källa ${färsk.noder.length} — kör: node tools/build-per-graph.mjs`);
check("den genererade grafen har samma kanter som källan",
  JSON.stringify(PER_GRAPH.kanter) === JSON.stringify(färsk.kanter),
  `genererad ${PER_GRAPH.kanter.length}, källa ${färsk.kanter.length} — kör: node tools/build-per-graph.mjs`);
/* Filen beskriver inte sig själv — annars hade varje generering ändrat sitt
   eget underlag och --check aldrig kunnat bli aktuell. */
check("den genererade filen är inte en nod i sig själv",
  !PER_GRAPH.noder.some(n => n.fil === "_per-graph-data.js"));
