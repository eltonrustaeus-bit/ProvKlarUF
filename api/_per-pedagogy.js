...
/** Läser förmågorna ur den genererade läroplanen. */
export function loadAbilities(sökväg) {
  try {
    const rå = readFileSync(sökväg, "utf8");
    const j = JSON.parse(rå);
    return Array.isArray(j.abilities) ? j.abilities : [];
  } catch {
    return [];
  }
}
